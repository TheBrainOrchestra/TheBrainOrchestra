...
// jitter matrices
var inMat = new JitterMatrix(1,"float32", 3, 3)
inMat.adapt = 1;

var outMat = new JitterMatrix(1,"float32", 3, 3);
var resultMat = new JitterMatrix(1,"long", 3, 1);
var minimasRows = new JitterMatrix(1,"float32", 1, 3);
var minimasCols = new JitterMatrix(1,"float32", 3, 1);
var minimasInterpolated = new JitterMatrix(1,"float32", 3, 3);
minimasInterpolated.interp = 1;
minimasInterpolated.adapt = 0;

// jitter operators
var jit_packer = new JitterObject("jit.pack");
var jit_unpack = new JitterObject("jit.unpack");
var jit_op = new JitterObject("jit.op");
var jit_expr = new JitterObject("jit.expr");
//jitExprAbs.expr = "abs(in[0])";
var jit_dimop = new JitterObject("jit.dimop");
...
// function that takes a weight matrix as arg 
function jit_matrix(inname)
{


	inMat.frommatrix(inname);
	resultMat.dim = [inMat.dim[0], 1];
	//post("resultMat dim", resultMat.dim, "\n")
	//minimasInterpolated.dim = inMat.dim;
	//minimasCols.dim = [inMat.dim[0], 1];
	//minimasRows.dim = [1, inMat.dim[1]];


	// take the absolute for weights matrix
	//jit_op.op = "abs";
	//jit_op.matrixcalc([inname, inname], outMat); // jit.op takes 2 args
	//
	/////*	SUBSTRACT MINIMAS PER ROW */
	//// find minimas per rows
	//jit_dimop.op = "min";
	//jit_dimop.step = [-1, 1];
	//jit_dimop.matrixcalc(outMat, minimasRows);
	//// interp minimasRows on NxN matrix
	//minimasInterpolated.frommatrix(minimasRows);
	//// substract minimasRows
	//jit_op.op = "sub";
	//jit_op.matrixcalc([outMat, minimasInterpolated], outMat); // jit.op takes 2 args

	///*	SUBSTRACT MINIMAS PER COLUMN */
	//// find minimas per columns
	//jit_dimop.op = "min";
	//jit_dimop.step = [1, -1];
	//jit_dimop.matrixcalc(outMat, minimasCols);
	//// interp minimasCols on NxN matrix
	//minimasInterpolated.frommatrix(minimasCols);
	//// substract minimasCols
	//jit_op.op = "sub";
	//jit_op.matrixcalc([outMat, minimasInterpolated], outMat); // jit.op takes 2 args

	//outlet(1,"jit_matrix",minimasInterpolated.name);

	//post(outMat.dim);

	// iterate on matrix

	//for(var iRow = 0; iRow < outMat.dim[1]; iRow++){
	//	for(var iCol = 0; iCol < outMat.dim[0]; iCol++){
	//	post("val", iRow, iCol, "=", outMat.getcell(iCol, iRow), "\n");
	//	}
	//}

	var matrix = [];
	var currentRow = [];
	for(var iRow = 0; iRow < inMat.dim[1]; iRow++){
		currentRow = [];
		for(var iCol = 0; iCol < inMat.dim[0]; iCol++){
		currentRow.push(inMat.getcell(iCol, iRow)); 
		//post("val", iRow, iCol, "=", outMat.getcell(iCol, iRow), "\n");
		}
	...
	//var matrix = [[5, 9, 1],
 	...
 	// fill jit.matrix with indices
  	for (var i = 0; i < indices.length; ++i) {
  	  var row = indices[i][0], col = indices[i][1];
  	  //var value = matrix[row][col];
  	  resultMat.setcell1d(indices[i][0], indices[i][1]);
  	  //total += value;
  	  //post('(' + row + ', ' + col + ') -> ' + value, "\n");
  	}
  	//post('total cost:', total, "\n");
  	outlet(0,"jit_matrix",resultMat.name);

}

function dim()
{
	// TODO : avoid changing dim every tick
}

////////////////////////////
// mukres algorithm from https://github.com/addaleax/munkres-js
